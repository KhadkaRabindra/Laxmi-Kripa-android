...
import com.maxx.eparchi.views.form.first_form.FirstForm2Fragment
import com.maxx.eparchi.views.unused.home.HomeActivity
import kotlinx.android.synthetic.main.bottom_button_view.*

class FourthForm1Fragment : BaseFragment() {
    @BindView(R.id.recyclerView)
    lateinit var recyclerview: RecyclerView

    companion object {
        fun newInstance(): FourthForm1Fragment {
            val fragment = FourthForm1Fragment()
            return fragment
        }
    }
    ...
    fun onViewClicked(view: View) {
        when (view.id) {
            R.id.bottomButton -> {
                openFragment(FourthForm2Fragment.newInstance())
            }
        }
    }
}